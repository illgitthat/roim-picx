<template>
  <div class="mx-auto max-w-6xl my-4 px-4 relative">
    ...
    <div class="flex items-center justify-between mb-4">
      <div>
        <div class="text-gray-800 text-lg">Manage Images</div>
        <div class="text-sm text-gray-500">
          Uploaded {{ uploadedImages.length }} images, total {{ formatBytes(imagesTotalSize) }}
        </div>
      </div>
    ...
    <div class="my-2 flex items-center justify-start flex-wrap"></div>
  ...
  <div class="grid gap-2 lg:gap-4 lg:grid-cols-4 grid-cols-2">
    <transition-group name="el-fade-in-linear">
      <div class="col-span-1 md:col-span-1" v-for="item in uploadedImages" :key="item.url">
        <image-box :src="item.url" :copyUrl="item.copyUrl" :name="item.key" :size="item.size" @delete="deleteImage(item.key)" mode="uploaded" />
      </div>
    </transition-group>
  </div>
  </div>
...
const addFolder = () => {
  ElMessageBox.prompt('Please enter the folder name, only English names are supported', 'Add Folder', {
